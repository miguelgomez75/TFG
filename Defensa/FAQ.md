...
1. [¿Por qué no usaste React o Vue?](#por-qué-no-usaste-react-o-vue)
2. [¿Por qué MongoDB y no PostgreSQL?](#por-qué-mongodb-y-no-postgresql)
3. [¿Qué pasa si el token JWT es robado?](#qué-pasa-si-el-token-jwt-es-robado)
4. [¿Por qué bcrypt y no SHA-256?](#por-qué-bcrypt-y-no-sha-256)
5. [¿Cómo añadirías un nuevo tipo de actividad?](#cómo-añadirías-un-nuevo-tipo-de-actividad)
6. [¿Usas WebSockets o algún mecanismo de actualización en tiempo real?](#usas-websockets-o-algún-mecanismo-de-actualización-en-tiempo-real)
7. [¿Cómo se protegen los datos clínicos de los pacientes?](#cómo-se-protegen-los-datos-clínicos-de-los-pacientes)
8. [¿Tiene el sistema tests automatizados?](#tiene-el-sistema-tests-automatizados)
9. [¿Por qué la API es REST y no GraphQL?](#por-qué-la-api-es-rest-y-no-graphql)
10. [¿Cómo escalaría el sistema si hubiera muchos usuarios simultáneos?](#cómo-escalaría-el-sistema-si-hubiera-muchos-usuarios-simultáneos)
11. [¿Qué es el patrón MVC y cómo se aplica aquí?](#qué-es-el-patrón-mvc-y-cómo-se-aplica-aquí)
12. [¿Qué diferencia hay entre autenticación y autorización, y cómo se implementan?](#qué-diferencia-hay-entre-autenticación-y-autorización-y-cómo-se-implementan)
13. [¿Por qué se usa `async/await` y no callbacks o promesas directas?](#por-qué-se-usa-asyncawait-y-no-callbacks-o-promesas-directas)
